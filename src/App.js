...
function App() {
  ...
  const AI = useMemo(() => new GeminiAI(SAFETY_CONFIGURATION), []);

  useEffect(() => {
    const initializeAI = async () => {
      try {
        await AI.ExtractData();
        AI.LoadKnowledge();
        AI.Train();
      } catch (error) {
  ...
  async function send_message(event) {
    ...
    let user_input = isRestartClicked && previousPrompt !== "" ? previousPrompt : inputBoxRef.current.value;

    if (user_input.length < 3) {
      setCurrentConversation([...currentConversation, "Please enter a valid prompt"]);
    } else {
      setCurrentConversation([...currentConversation, user_input]);

      await AI.Send(user_input);
      const response = await AI.Receive();
          ...
          <div className="ww-w-[15rem] ww-h-[30rem] ww-bg-[#121212] ww-rounded-lg ww-relative">
            <div className="ww-flex ww-flex-row ww-text-neutral-400">
              ...
              <button onClick={() => setIsCollapsed(true)}>
                <span className="ww-absolute ww-text-[0.5rem] ww-right-7 ww-translate-y-[13px]">Off</span>
            ...
            <div className="ww-absolute ww-w-11/12 ww-h-[14rem] ww-top-12 ww-left-2">
              <div className="ww-w-full ww-h-full ww-overflow-y-auto ww-hide-scrollbar ww-flex ww-flex-col" ref={chatBoxRef}>
            ...
            <div className="ww-absolute ww-bottom-[11.5rem] ww-right-4 ww-text-neutral-400 ww-flex">
              <button onClick={() => { setIsRestartClicked(true); send_message(); }}>
                <VscDebugRestart />
              </button>
            </div>
            <form

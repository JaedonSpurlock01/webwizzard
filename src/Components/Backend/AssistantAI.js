...
const InstagramScrapper = require('./InstagramScrapper'); 
...
const INSTAGRAM_API_URL = 'https://graph.instagram.com/v11.0';
...
export class GeminiAI {
  ...
  ExtractData(){
      ...
      console.log("Dados extraídos:", this.WebScrapper.data);

  }

  LoadKnowledge(){ 
    const siteData = this.WebScrapper.data['merged'];
    const chunks = siteData.map(item => new Chunk(JSON.stringify(item), 'merged'));
    this.__KNOWLEDGE.push(...chunks);
    this.AI_TRAINER.LoadKnowledge(this.__KNOWLEDGE);
    console.log("Chunks carregados:", this.__KNOWLEDGE);
  }
